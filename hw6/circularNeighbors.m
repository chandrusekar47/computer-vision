...
%     % right/bottom edge is less than radius
    bottom_right_x = ceil(center_x + radius);
    bottom_right_y = min(height, ceil(center_y + radius));
...
%     relative_center_y = center_y - top_left_y + 1;
    [X, Y] = meshgrid(top_left_x:bottom_right_x, top_left_y:bottom_right_y);
    % get all points inside the circle
    circle_points = ((X-center_x).^2 + (Y-center_y).^2 ) < radius^2;
    % create a bounding box around the circle and compute the feature matrix
    % for all pixels in the bounding box
    bounding_box_feature_matrix = generate_feature_matrix(img, top_left_x, top_left_y, bottom_right_x - top_left_x + 1, bottom_right_y - top_left_y + 1);
    ...
    feature_matrix(:, 1) = feature_matrix(:, 1) + top_left_x;
    feature_matrix(:, 2) = feature_matrix(:, 2) + top_left_y;
end